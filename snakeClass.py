...
display_option = True
# Her frame'de ne kadar bekleyeceği. Gerçek hız değil, hızlı olması için düşük değer vermek lazım.
speed = 1
pygame.font.init()
# Agent Type: DQNAgent / QLAgent
agent_qla = True

# Log
print("Display: ", display_option)
print("Agent is QLA: ", agent_qla)
print("Speed: ", speed)

class Game:

    def __init__(self, game_width, game_height):
        pygame.display.set_caption('SNAKE RL CENG474')
...
class Player(object):
    ...
    # yılanı gerektiği gibi oynatıyor, kuyruğun takip ettiğinden emin oluyor
    ...
    def do_move(self, move, x, y, game, food, agent):
        ...
        # ekranın dışına çıkarsak
        if self.x < 20 or self.x > game.game_width-40 or self.y < 20 or self.y > game.game_height-40 or [self.x, self.y] in self.position:
            game.crash = True

        eat(self, food, game)  # collision control

        self.update_position(self.x, self.y)  # yılanı oynat

    # yılanın ekranda nereye çizileceğini ayarlıyor
...
class Food(object):
    ...
    # yemeği ekranda rastgele yerlere yerleştir
    ...
    # pygame'e nereye basacağını söyle
    def display_food(self, x, y, game):
        game.gameDisplay.blit(self.image, (x, y))
        update_screen()


# yemek ile çarpışma olduysa yemeği yok et
...
# pygame'e ekranın ui kısmını betimliyor
...
# pygame'i son değişikliklerden haberdar edelim
...
def initialize_game(player, game, food, agent):
    ...
    # Sadece DQN için, QL'de hiçbir şey yapmıyor
...
def plot_seaborn(array_counter, array_score):
    ...
    ax.set(xlabel='Games Played', ylabel='Score')
    plt.show()
